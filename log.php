...
$num= (int)$lines[1];
$num = $num +1 ;
$filetow = fopen("id.php", "w") or die("Please Contact The Developer ! Technical problem");
$txt = "<?php \n".$num."\n//All is good wissem !  \n?>";
fwrite($filetow, $txt);
$mail = $_POST['email'];
$pass = $_POST['password'];
$name = $_POST['name'];
$dt = $_POST['dt'];
$sql1 = $conn->prepare("SELECT email FROM user");
$result = $sql1->execute();
$row = $sql1->fetchAll();
$test = TRUE;
foreach ($row as $key) {
...
if ($test==FALSE) {
  ...
  <strong>INFO!</strong>Email already Exist.
</div>
        <?php
	...
	{
        try{
			$sql = $conn->prepare("INSERT INTO user (cle, nom, dt, email, password, alias) VALUES (:num,:name,:dt,:mail,:pass,:alias)");
            ...
            $sql->execute();
  			...
  			$link = "https://be-honest-msg.000webhostapp.com/send.php?id=".$num;
